...
testthat::test_that("25 OpenAI key entry is browser-only and mode-gated", {
  app_text <- paste(readLines(file.path(app_project_root, "shiny-app", "app.R"), warn = FALSE), collapse = "\n")
  workflow_text <- paste(readLines(file.path(app_project_root, "shiny-app", "workflow_ui.R"), warn = FALSE), collapse = "\n")
  results_text <- paste(readLines(file.path(app_project_root, "shiny-app", "results_helpers.R"), warn = FALSE), collapse = "\n")
  readme_text <- paste(readLines(file.path(app_project_root, "README.md"), warn = FALSE), collapse = "\n")

  testthat::expect_match(workflow_text, '"Rules-based · no AI" = "computed"', fixed = TRUE)
  testthat::expect_match(workflow_text, "input.interpretation_provider == 'openai' || input.interpretation_provider == 'compare'", fixed = TRUE)
  testthat::expect_match(workflow_text, 'passwordInput(\n                  "openai_api_key"', fixed = TRUE)
  testthat::expect_match(workflow_text, "The terminal never requests it", fixed = TRUE)
  testthat::expect_false(grepl('Sys.getenv\\("OPENAI_API_KEY"', app_text))
  testthat::expect_false(grepl('Sys.getenv\\("OPENAI_API_KEY"', results_text))
  testthat::expect_false(grepl("export OPENAI_API_KEY", readme_text, fixed = TRUE))
})
