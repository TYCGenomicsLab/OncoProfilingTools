...
testthat::test_that("Compare mode preserves both providers and selects OpenAI when both complete", {
  with_test_openai_key({
    ...
    tabs <- report_interpretation_tab_views(bundle)
    testthat::expect_identical(tabs$current, "openai")
    testthat::expect_true(tabs$views$openai$available)
    testthat::expect_true(tabs$views$ollama$available)
    testthat::expect_match(tabs$views$openai$provenance, "OpenAI / gpt-5.6-terra", fixed = TRUE)
    testthat::expect_match(tabs$views$ollama$provenance, "Ollama / llama3.1:8b", fixed = TRUE)
    testthat::expect_match(tabs$views$technical$provenance, "Rules-based / deterministic", fixed = TRUE)
    comparison_html <- report_provider_comparison_html(bundle, "go")
    testthat::expect_match(comparison_html, "Interpretation provider diagnostics", fixed = TRUE)
...
testthat::test_that("failed provider comparisons do not present computed prose as OpenAI output", {
  with_test_openai_key({
    ...
    testthat::expect_match(html, "OpenAI Premium ended · computed summary available", fixed = TRUE)
