...
#' Find a metadata sidecar file beside an assay file using a filename pattern
#'
#' @param path Path to the primary assay file.
#' @param pattern Regular expression used to match the metadata filename.
#'
#' @return A normalized character path if exactly one match is found, otherwise
#'   `NULL` if no match is found.
#'
#' @keywords internal
.find_sidecar_file_pattern <- function(path, pattern) {
  if (!is.character(path) || length(path) != 1L || is.na(path) || !nzchar(path)) {
    stop("`path` must be a single non-empty character string.", call. = FALSE)
  }
  ...
  matches <- candidate_files[
    grepl(pattern, basename(candidate_files))
  ]
  ...
  if (length(matches) > 1L) {
    stop(
      "Multiple metadata sidecar files matched pattern `",
      pattern,
      "` beside assay file: ",
