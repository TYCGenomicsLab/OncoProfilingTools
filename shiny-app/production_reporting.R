...
build_combined_html_report <- function(
  ...
  html <- paste0(
    ...
    "</main><script>(function(){const areas=Array.from(document.querySelectorAll('.researcher-editable'));const toggle=document.getElementById('toggle-edit');const download=document.getElementById('download-edited');document.querySelectorAll('.interpretation-switcher').forEach(function(container){const stateNode=container.querySelector('.interpretation-tab-data');const shared=container.querySelector('.shared-interpretation-box');const provenance=container.querySelector('.interpretation-view-provenance');const tabButtons=Array.from(container.querySelectorAll('.interpretation-view-button'));let tabState=null;try{tabState=stateNode?JSON.parse(stateNode.textContent):null;}catch(error){tabState=null;}let current=tabState?tabState.current:null;function syncStateNode(){if(stateNode&&tabState){stateNode.textContent=JSON.stringify(tabState).replace(/</g,'\\u003c');}}function saveCurrentInterpretation(){if(tabState&&current&&shared&&tabState.views[current]){tabState.views[current].html=shared.innerHTML;tabState.current=current;syncStateNode();}}function showInterpretation(key){if(!tabState||!tabState.views[key]||!shared)return;saveCurrentInterpretation();current=key;tabState.current=key;shared.innerHTML=tabState.views[key].html;if(provenance){provenance.textContent=tabState.views[key].provenance||'';provenance.hidden=!tabState.views[key].provenance;}tabButtons.forEach(function(button){const active=button.dataset.interpretationView===key;button.classList.toggle('active',active);button.setAttribute('aria-selected',active?'true':'false');});syncStateNode();}tabButtons.forEach(function(button){button.addEventListener('click',function(){showInterpretation(button.dataset.interpretationView);});});if(shared){shared.addEventListener('input',saveCurrentInterpretation);}});let editing=false;if(toggle){toggle.addEventListener('click',function(){editing=!editing;areas.forEach(function(node){node.setAttribute('contenteditable',editing?'true':'false');});toggle.textContent=editing?'Finish text editing':'Enable researcher text editing';});}if(download){download.addEventListener('click',function(){document.querySelectorAll('.interpretation-switcher').forEach(function(container){const shared=container.querySelector('.shared-interpretation-box');if(shared){shared.dispatchEvent(new Event('input'));}});const clone=document.documentElement.cloneNode(true);clone.querySelectorAll('[contenteditable]').forEach(function(node){node.removeAttribute('contenteditable');});const blob=new Blob(['<!doctype html>\\n'+clone.outerHTML],{type:'text/html;charset=utf-8'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download='OncoProfiling_Edited_Report.html';document.body.appendChild(link);link.click();link.remove();setTimeout(function(){URL.revokeObjectURL(url);},1000);});}})();</script></body></html>"
  )
